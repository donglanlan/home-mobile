  ...
  <div class="cube-upload">
    <slot>
      <div class="cube-upload-def clear-fix">
        <upload-file v-for="(file, i) in files"
          :file="file"
          :key="i"
          @click="fileClick"></upload-file>
        <!-- <upload-btn v-show="isShowBtn"></upload-btn> -->
        <upload-btn1 v-if="frontFlag && isShowBtn"></upload-btn1>
        <upload-btn2 v-if="backFlag && isShowBtn"></upload-btn2>
      </div>
    </slot>
  </div>
</template>
<script type="text/ecmascript-6">
import UploadBtn1 from './btn.vue'
import UploadBtn2 from './btn2.vue'
...
const EVENT_ADDED_FILEENTRY = 'fileEntry-added'
...
export default {
  ...
  props: {
    ...
    max: {
      type: Number,
      default: 1 // default改为1,添加图片时upload-btn不显示
    },
    auto: {
      type: Boolean,
      default: true
  ...
  methods: {
    addFileEntry(fileEntry) {
      this.$emit(EVENT_ADDED_FILEENTRY, fileEntry)
      fileEntry.file((file) => {
        this.addFiles([file])
      })
    },
